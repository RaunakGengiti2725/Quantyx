...
class BulkTree:
    ...
    def compute_single_curvature(self, node: str, weights: list[float]) -> float:
        """
        Compute curvature for a single node based on its local topology and edge weights.
        This provides more physics-based curvature calculation.
        """
        if node not in self.tree:
            return 0.0
            
        # Get node's degree (number of connections)
        degree = self.tree.degree[node]
        if degree == 0:
            return 0.0
            
        # Get edge weights for edges connected to this node
        connected_edges = [(u, v) for u, v in self.tree.edges if u == node or v == node]
        
        if not connected_edges:
            return 0.0
            
        # Calculate topology-based curvature
        # Higher degree nodes have different curvature characteristics
        edge_weight_sum = 0.0
        for edge in connected_edges:
            edge_idx = self.edge_to_index.get(edge, 0)
            if edge_idx < len(weights):
                edge_weight_sum += weights[edge_idx]
        
        # Discrete curvature formula inspired by graph theory
        # K(v) = 1 - (degree(v) / 2) + (weighted_sum / degree(v))
        if degree > 0:
            avg_weight = edge_weight_sum / degree
            curvature = 1.0 - (degree / 4.0) + avg_weight
        else:
            curvature = 0.0
